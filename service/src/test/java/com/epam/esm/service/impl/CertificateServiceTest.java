...
class CertificateServiceTest {
    ...
    private final static String FIRST_NAME = "Apple.com";
    private final static BigInteger FIRST_ID = BigInteger.ONE;
    private final static BigInteger SECOND_ID = BigInteger.TWO;
    private final static String SECOND_NAME = "OZ.com";
    private final static String DESCRIPTION = "Certificate description";
    private final static BigDecimal PRICE = BigDecimal.TEN;
    private final static int DURATION = 42;
    private final static String TAG_NAME = "#cool";
    private final static String SORT_BY_NAME = "ASC";
    private final static LocalDateTime CREATE_DATE = LocalDateTime.now();
    private final static LocalDateTime LAST_UPDATE_DATE = LocalDateTime.now();
    private final static int DAO_UPDATE_RETURN_VALUE = 3;
    private final Certificate firstCertificate =
            new Certificate(FIRST_ID, FIRST_NAME, DESCRIPTION, PRICE, DURATION, CREATE_DATE, LAST_UPDATE_DATE);
    private final Certificate secondCertificate =
            new Certificate(SECOND_ID, SECOND_NAME, DESCRIPTION, PRICE, DURATION, CREATE_DATE, LAST_UPDATE_DATE);
    ...
    public void findAllWithTagsByParamsTest() {
        final List<Certificate> certificates = List.of(firstCertificate, secondCertificate);
        when(tagService.findAll(FIRST_ID)).thenReturn(List.of(new TagDTO(BigInteger.ONE,TAG_NAME)));
        when(certificateDAO.findAll(FIRST_NAME, TAG_NAME, DESCRIPTION, "ASC", null)).thenReturn(certificates);
        when(certificateConverter.convert(any(Certificate.class))).thenReturn(
                new CertificateDTO(FIRST_ID, FIRST_NAME, DESCRIPTION, PRICE, DURATION, CREATE_DATE, LAST_UPDATE_DATE));
        final Collection<CertificateDTO> certificateDTOS = certificateService.findAll(FIRST_NAME, TAG_NAME, DESCRIPTION, "ASC", null);
        for (CertificateDTO certificateDTO : certificateDTOS){
            System.out.println(certificateDTO+" "+certificateDTO.getTagsDTO());
        }
        assertNotNull(certificateDTOS);
        assertFalse(certificateDTOS.isEmpty());
        assertTrue(
                certificateDTOS.stream().anyMatch((p) -> p.getTagsDTO().get(0).getName().equals(TAG_NAME)));
    }
    ...
    public void updateTest() throws ValidatorException {
        ...
        assertEquals(certificateService.update(any(BigInteger.class), new CertificateDTO()), DAO_UPDATE_RETURN_VALUE);
    }
